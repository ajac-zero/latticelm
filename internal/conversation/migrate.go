...
// expectedSchemaVersion is the version that NewSQLStore requires at startup.
...
// a positive integer (e.g. 001_create_conversations.sql).
...
// createSchemaMigrationsTable creates the schema_migrations tracking table if
// it does not already exist. The DDL is compatible with SQLite, PostgreSQL, and
// MySQL.
...
// recordMigration inserts a row into schema_migrations inside the given
// transaction.
func recordMigration(ctx context.Context, tx *sql.Tx, driver string, m migration) error {
	var q string
	if driver == "pgx" || driver == "postgres" {
		q = `INSERT INTO schema_migrations (version, description, applied_at) VALUES ($1, $2, $3)`
	} else {
		q = `INSERT INTO schema_migrations (version, description, applied_at) VALUES (?, ?, ?)`
	}
