...
func (s *InstrumentedStore) Create(ctx context.Context, id string, model string, messages []api.Message, owner conversation.OwnerInfo, request *api.ResponseRequest, replayState *api.ReplayState) (*conversation.Conversation, error) {
	// Start span if tracing is enabled
	if s.tracer != nil {
		var span trace.Span
	...
	// Call underlying store
	conv, err := s.base.Create(ctx, id, model, messages, owner, request, replayState)
